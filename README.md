...
1. 打开站点底部的「資料を編集 / 内容编辑」。搜索现有条目或选择「新建条目」。人物介绍也在资料列表中，可以编辑姓名、所属作品、介绍、伙伴、台词和贴纸。
2. 新建提供 11 种模板：星光舞台卡面、灰姑娘女孩卡面、服装、歌曲、剧情、视频剧情、视频、组合、资讯、足迹、人物介绍。也可搜索并完整复制已有页面；文字、图片、表格、关联和扩展资料均保留，只生成新的条目 ID。
3. 点击「边看边编辑」。左侧直接加载主站页面，右侧为双语表单。点击预览里的标题、简介、图片、制作人员、数值、章节等内容，会定位对应字段；修改自动保存并实时显示。电脑、平板、手机按钮调整预览宽度，日语和中文可独立预览。关闭「点选内容修改」可检查排版；卡面、Q 版姿态、贴纸和语音分组仍可切换。
4. 图片从素材库选择或直接上传，无需填写文件路径。PNG / JPG / WebP 每张最多 8 MB，贴纸另支持 GIF。正文、信息表、曲目表、台词与图集都支持添加、删除和调整顺序。未展示在页面上的附加资料仍保留在表单里。
5. 点击「检查并提交」，查看每条修改的旧值和新值。编辑器会检查必填字段、图片、关联及最新公开数据；同一字段被其他编辑者修改时，会先要求处理冲突。
6. 首次提交按「首次连接指南」创建 GitHub 连接密钥：只选择 `YoshinoDB` 仓库，授予 **Contents: Read and write** 和 **Pull requests: Read and write**，然后粘贴到编辑器。密钥仅用于本次连接，不会写入浏览器草稿、仓库或发布包。你需要拥有该仓库的写入权限。
7. 连接后会再次核对 **GitHub main 的最新提交**，展示实际将提交的修改。点击「确认修改并创建 PR」后，文字、图片和素材清单一次提交到独立分支。打开返回的 PR，检查通过后在 GitHub 合并，现有 Pages 工作流会自动部署主站。关闭窗口或断线后重新连接，会先查找上次提交，避免重复创建提案。

仍可直接下载发布包 ZIP，解压后把 `public` 文件夹拖入 GitHub 上传页；没有新图片时也能单独下载 `catalog.json`。这是没有仓库 API 写入权限、或希望交给维护者处理时的备用方式。

草稿属于当前浏览器，保存和预览不会更新公开网站。图片保存在本机 IndexedDB，刷新后仍可恢复。旧草稿可继续读取；导入按 ID 合并，不删除文件中没有的条目。结构操作可撤销上一步，已有条目的 ID 固定。高级 JSON 只是可选工具，不是日常编辑所必需；暂存的 JSON 必须应用或放弃后才能提交。

试听音源仍由 Apple 官方接口自动核对。歌曲「官方试听」字段可直接粘贴 Apple Music 的**歌曲分享链接**（不是整张专辑）；下次自动更新核对歌名和演唱者后加入试听，清空链接则隐藏。已有歌曲的分享链接已迁入资料数据；复制原页面时也会保留。

实现与维护说明见 [内容编辑与发布](docs/content-editor.md)。
...
歌曲条目的 `music.appleTrackUrl` 保存人工选择的 Apple Music 歌曲链接，可直接通过编辑器填写。`config/listening.json` 保留旧版映射作为兼容来源。新收录歌曲不会猜测匹配同名音源；运行 `npm run sync:music` 或等待每日同步后，脚本核实对应曲目。脚本一次批量查询并核对歌名、演唱者和版本，将结果写入 `public/data/listening.json`。接口故障保留已有数据；单首撤下时保留上次地址和可用的商店入口，不影响歌曲条目的收录。当前《日々あどべんちゃーなのでしてー》为 GAME VERSION 试听，《桜の風》保留官方视频入口。

背景音乐使用维护者提供的三首完整原版伴奏：《日々あどべんちゃーなのでしてー》（5:37）、《祈りの花》（4:58）和《桜の頃（M@STER VERSION）》（4:26）。歌单、曲名和唱片出处统一在 `config/bgm.json` 维护。原 Ogg 文件转为 AAC / M4A，并移除文件附带元数据，保存在 `public/audio/`，以兼容 Safari 等浏览器。著作权归相关权利人所有。

右下角只有一个 BGM 开关，默认开启，三首歌每轮随机排序，每首播放一次后重新打乱，轮次交界处也不会连续重复同一首；音量固定为轻柔的 22%。浏览器允许时自动播放；被有声自动播放策略拦截时，在第一次点击页面或按 Enter／空格后启动。跨站内页面连续播放，刷新或重新进入网站重新随机选择第一首，从曲首播放，仅记住开关偏好。手动关闭会保存在浏览器，之后的点击、刷新和媒体结束都不会擅自重新开启。歌曲试听和卡片语音期间暂停 BGM，播放暂停、结束或载入失败后恢复；视频展开期间暂停 BGM，收起视频或离开该页后恢复。替换背景音乐时更新音频文件和 `config/bgm.json`；关于本站的曲目信息自动同步。
...
editor.js / editor.css          表单、模板选择、实时预览和 PR 提交
lib/content-templates.js        11 种内容模板和无损复制
lib/editor-fields.js            渲染字段到表单的定位
lib/editor-preview.js           主站预览与点选编辑桥接
lib/editor-publish.js           最新提交核对、图片提交、PR 与断线恢复
...
仓库 **Settings → Pages → Source** 选择 **GitHub Actions**。推送到 `main` 或手动运行 `Deploy to GitHub Pages` 即可。拉取请求运行 `Check proposed content` 的数据检查、测试与构建，不触发生产部署；合并后才由 main 的 Pages 工作流发布。网站不需要后台、数据库或付费服务。可用 GitHub 历史提交恢复内容。
...
**非营利声明不等于获得素材使用许可。** 官方页面未提供开放再利用授权；图片、角色名称、音乐等权利归各自权利人。网站保留版权标注和权利人删除/更正联络入口。视频使用平台提供的嵌入播放器，文章提供摘要和原文链接。CGSS 台词、剧情及小剧场由站主提供的保存页导入，保留原文、译文与来源；语音点击后直接播放 BWIKI 的 MP3 文件，不在仓库存放音频副本。导入与编辑方法见 [CGSS 台词与剧情](docs/cgss-dialogue.md)。
...
卡面 `card` 包含特技、队长效果和特训前后最大等级／亲爱度数值；`gallery` 保存两版插画。歌曲 `music` 包含演唱者、作词作曲编曲、唱片编号与曲目表。视频 `video` 可设置投稿者、时长、系列和 `parts` 分集；剧情 `story` 支持出演成员、活动期间与 `chapters` 篇章链接。组合 `unit` 包含成员、首次组合活动和代表曲。资讯可用 `sections` 增加段落、信息表，并用 `gallery` 展示更多图片。`relatedIds` 可以指定相关条目。页面中的内容编辑器提供这些扩展字段的结构化表单，保留未修改的字段；管理分组也提供可选的高级 JSON 编辑。人物身份文案位于 `profile.identity`。
