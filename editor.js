...
export function mountEditor(root, catalog, { t, tr, esc, label }) {
  ...
  root.querySelector("#preview-record").onclick = () => {
    ...
    try {
      ...
      validateCatalog({ version: 1, items: [item] });
      root.querySelector("#draft-preview").innerHTML =
        `<article class="draft-card"><small>${label(item.kind)} · ${item.official ? t("公式", "官方") : t("有志資料", "粉丝资料")}</small><h2>${esc(tr(item.title))}</h2><p>${esc(tr(item.description))}</p><p>${esc(item.date)} · ${esc(item.sourceName)}</p></article>`;
    } catch (error) {
      say(error.message);
    }
  };
