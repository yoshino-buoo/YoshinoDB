...
export async function imageStore(method, value) {
  // WebKit can abort IndexedDB transactions containing filesystem-backed
  // File/Blob objects. Store plain bytes; also keep reading older Blob drafts.
  if (method === "put" && value.blob) {
      ...
      resolve(
        method === "getAll"
          ? request.result.map((row) => ({
              ...row,
              blob: row.blob || new Blob([row.bytes], { type: row.mimeType }),
            }))
          : request.result,
      );
    tx.onerror = () =>
      reject(tx.error || request.error || Error("Image storage interrupted"));
