import { appleTrackId } from "./listening.js";
...
export function entryIssues(item, ids = new Set(), zh = false) {
  ...
  if (
    ...
    add(
      "game",
      "ゲームを deresute / mobamas から選んでください",
      "请选择所属游戏：deresute / mobamas",
    );
  if (item.music?.appleTrackUrl && !appleTrackId(item.music.appleTrackUrl))
    add(
      "music.appleTrackUrl",
      "Apple Music の曲の共有リンクを入力してください",
      "请粘贴 Apple Music 的歌曲分享链接，不能使用整张专辑的链接",
    );
  const walk = (value, path = "") => {
    ...
    for (const [key, v] of Object.entries(value)) {
      ...
      if (
        ...
        !(
          p.startsWith("profile.stickers.") &&
          /^assets\/[a-zA-Z0-9_.-]+\.gif$/.test(v)
        )
      )
  ...
  for (const prefix of ["voiceGuide", "commu", "card.petit"]) {
    const value = getPath(item, prefix);
    if (value && !isHttps(value.source))
      add(
        `${prefix}.source`,
        "この部分の https:// 掲載ページを入力するか、部分ごと取り除いてください",
        "请填写此部分的 https:// 来源链接，或点击“移除此部分”",
      );
  }
  const checkArt = (art, path) => {
    if (!art?.image) add(`${path}.image`, "画像を選んでください", "请选择图片");
    if (!isHttps(art?.source))
      add(
        `${path}.source`,
        "画像の掲載ページを入力してください",
        "请填写图片来源页面",
  ...
  if (Array.isArray(item.profile?.stickers))
    item.profile.stickers.forEach((art, i) =>
      checkArt(art, `profile.stickers.${i}`),
    );
  if (Array.isArray(item.theater?.panels))
    item.theater.panels.forEach((panel, i) => {
      checkArt(panel.ja, `theater.panels.${i}.ja`);
      if (panel.zh) checkArt(panel.zh, `theater.panels.${i}.zh`);
    });
  for (const [i, stage] of (Array.isArray(item.voiceGuide?.stages)
    ? item.voiceGuide.stages
    : []
  ).entries())
    for (const [j, group] of (Array.isArray(stage.groups)
      ? stage.groups
      : []
    ).entries())
      for (const [k, clip] of (Array.isArray(group.clips)
        ? group.clips
        : []
      ).entries())
        if (!/^CGSS-[a-zA-Z0-9_+-]+\.mp3$/.test(clip.file || ""))
          add(
            `voiceGuide.stages.${i}.groups.${j}.clips.${k}.file`,
            "BWIKI の音声ファイル名（CGSS-…mp3）を入力してください",
            "请填写 BWIKI 语音文件名（CGSS-…mp3）",
          );
  try {
    ...
    if (!issues.length) {
      const paths = {
        "profile sticker": "profile.stickers",
        "petit idol": "card.petit.poses",
        "voice guide": "voiceGuide.stages",
        "voice groups": "voiceGuide.stages",
        "voice file index": "voiceGuide.stages",
        "card commu": "commu.lines",
        "card theater": "theater.episode",
        "card stats": "card.stats",
        "Mobamas stats": "card.mobamasStats",
      };
      const path = Object.entries(paths).find(([message]) =>
        error.message.includes(message),
      )?.[1];
      add(
        path || "advanced",
        path
          ? "この部分の未入力・形式を確認してください。不要な場合は取り除けます。"
          : `データを確認してください：${error.message}`,
        path
          ? "请检查此部分的未填项或格式；不需要时可移除此部分。"
          : `请检查数据：${error.message}`,
      );
    }
  }
