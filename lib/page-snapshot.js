...
// A closed shadow tree isolates the inert outgoing view from application selectors,
...
export function capturePage(previous) {
  ...
  originals.forEach((element, index) => {
    ...
    if (element instanceof HTMLImageElement) artwork.push([element, clone]);
  });
  // A cached lazy image can report complete in Safari while its new clone still
  // paints empty for several frames. Keep the already displayed image itself.
  // Move only after measuring the entire page so image removal cannot change
  // the layout or animated poses captured for subsequent elements.
  for (const [image, clone] of artwork) {
