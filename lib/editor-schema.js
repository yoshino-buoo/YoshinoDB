      ...
      rows("voiceGuide.stages", "段階・テーマ", "阶段与主题", [
        ...
        rows("groups", "ボイスの場面", "语音场景组", [
          ...
          rows("clips", "音声の入口", "语音入口", [
            ...
            f("audio", "BWIKIのMP3直リンク", "BWIKI MP3 直链", "url"),
            f(
              "file",
...
export const DETAILS = {
  cards: [
    ...
    block(
      ...
      "Q 版芳乃",
      [
        f("card.petit.source", "掲載ページ", "来源页面", "url"),
        rows("card.petit.poses", "ポーズ（最大4枚）", "姿态图片（最多四张）", [
          f("image", "画像", "姿态图片", "image"),
    ...
    block(
      ...
      "特训剧情",
      [
        f("commu.source", "掲載ページ", "剧情页面", "url"),
        f("commu.videoUrl", "動画リンク", "剧情视频链接", "url"),
        rows(
          "commu.lines",
          "台本（表示順）",
          "完整对话（按显示顺序）",
    ...
    block(
      ...
      "小剧场 WIDE☆",
      [
        f("theater.episode", "話数", "期数", "number"),
        f("theater.translationCredit.name", "翻訳者", "汉化者"),
        f(
          "theater.translationCredit.url",
          "翻訳者のページ",
          "汉化者主页",
          "url",
        ),
        rows(
          ...
          [
            ...
            ...[
              ["ja", "日本語", "日语"],
              ["zh", "中国語", "中文"],
            ].flatMap(([locale, ja, zh]) => [
              f(`${locale}.image`, `${ja}の画像`, `${zh}图片`, "image"),
              f(
                `${locale}.source`,
                `${ja}の掲載ページ`,
                `${zh}图片来源`,
              ...
              f(`${locale}.width`, `${ja}の画像幅`, `${zh}图片宽度`, "number"),
              f(
                `${locale}.height`,
                `${ja}の画像高さ`,
                `${zh}图片高度`,
          ...
          { part: 1, ja: { width: 1280, height: 720 } },
        ),
      ],
      true,
      "deresute",
  ...
  songs: [
    block(
      ...
      [
        f(
          "music.appleTrackUrl",
          "Apple Music の曲リンク（アルバムではなく曲を共有）",
          "Apple Music 歌曲分享链接（请选择歌曲，非整张专辑）",
          "url",
          {
            hint: [
              "曲名と歌手を確認後、次回の自動更新で試聴が反映されます。空欄で非表示。",
              "填写后会核对歌名与演唱者，在下次自动更新时加入试听；清空则隐藏。",
            ],
          },
        ),
      ],
  ...
  profile: [
    block("人物の表紙", "人物页头", [
      l("profile.identity.name", "表示する名前", "显示姓名"),
      f("profile.identity.romanized", "ローマ字表記", "罗马字姓名"),
      l("profile.identity.franchise", "作品名", "所属作品"),
      f("profile.identity.type", "タイプ", "属性"),
      l("profile.identity.imageAlt", "立ち絵の説明", "立绘文字说明"),
    ]),
    block(
      "スタンプとアニメーション",
      "贴纸与动图",
      [
        rows(
          ...
          "贴纸",
          [
            f("group", "シリーズ", "所属系列", "text", {
              options: ["mobamas", "deresute", "line"],
            }),
            l("label", "名前", "名称"),
            f("image", "画像（GIF対応）", "图片（支持 GIF）", "image"),
            f("source", "掲載ページ", "来源页面", "url"),
            f("width", "画像幅", "图片宽度", "number"),
            f("height", "画像高さ", "图片高度", "number"),
          ],
          {
            group: "deresute",
            label: { ja: "", zh: "" },
